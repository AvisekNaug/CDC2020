...
ts_seq2seq_model: learns the forecast problem as a many to many sequence prediction problem

hybrid_seq2seq_model: learns the forecast problem as a many to many sequence prediction problem 
with varying modes. self.hybrid_modes is used to specify possible number of modes as a result of 
combination of multiple variable mode variation.
...
from keras.layers import Input, Dense, LSTM, RepeatVector, Reshape, Dropout, multiply, \
...
class hybrid_LSTM_model():
	...
	dropoutlist: list = [[],[]], batchnormalizelist : list = [[],[]]):
		...
		self.fc1 = Dense(8, activation='relu')(self.input_layer)
		self.fc2 = Dense(8, activation='relu')(self.fc1)
		self.sfmax = Dense(2, activation='softmax')(self.fc2)
		...
		self.model = Model(inputs=self.input_data, outputs=[self.input_layer, self.sfmax])

	def model_compile(self,):
		# compile model
		self.model.compile(loss=['mse', 'binary_crossentropy'],
	...
	def evaluate_model(self, X_test, y_test, y_sc, save_plot_loc,
		...
		# evaluate model on data. output -> (nsamples, output_timesteps, outputdim)
		# self.preds_train = self.model.predict(X_train, batch_size=self.batch_size)
		self.preds_test, self.binaryclass = self.model.predict(X_test, batch_size=self.batch_size)
		self.binary_idx = np.argmax(self.binaryclass, axis=-1).reshape(-1,1,1)
		self.preds_test = np.multiply(self.preds_test, self.binary_idx)
		...
		return self.preds_test, self.binary_idx